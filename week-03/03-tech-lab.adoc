...
Notice that you can run directly, but first call 'docker ps' to see if port 8081 has already been bound.  If it is, call "docker kill" to un-bind it.

$ docker run -d -p 8081:30100 gcr.io/<Google Project ID>/node-svc:latest
