...
Create a Terraform configuration file called `main.tf` inside the `2-terraform` directory with the following content:
...
$ scp -r ../../01/install.sh node-user@${NODE_IP_01}:/home/node-user # copy install script
$ rsh ${NODE_IP_01} -l node-user chmod +x /home/node-user/install.sh # set permissions
$ rsh ${NODE_IP_01} -l node-user /home/node-user/install.sh # install app
$ rsh ${NODE_IP_01} -l node-user sudo nodejs /home/node-user/node-svc/server.js  # run app
----
...
=== Add more GCP resources into Terraform
...
Your version of `main.tf` file should now look similar to this (change the ssh key file path, if necessary):
...
Now, using the same techniques as above, verify that the application became accessible again on port 3000 (locally and remotely) and SSH connection with a private key works.
...
Save and commit the `2-terraform` folder created in this lab into your course repo.
...
Create a new directory called `3-ansible` inside your course repo, which we'll use to save the work done in this lab.
...
[source,bash]
----
$ cd ../2-terraform  # adapt this command as necessary to get to the directory
$ terraform apply -auto-approve
----
...
Here we define a group of hosts (`node-svc-grp`) under which we list the hosts that belong to this group. For now, we list only one host under the hosts group and give it a name (`node-svc-01`) and information on how to connect to the host.
...
Prove your work by issuing a URL of /2 or above to the microservice on either machine and posting the output to Teams.
