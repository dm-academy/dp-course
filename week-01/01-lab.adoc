...
In the previous section, you deployed the https://github.com/dm-academy/node-svc[node-svc] application by connecting to a VM via SSH and running commands in the terminal one by one.
...
In the 01 directory create a script `provision.sh`:
...
In the 01_2-script directory create a script `deprovision.sh`.
