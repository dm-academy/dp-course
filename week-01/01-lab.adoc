...
You have developed a new cool application called https://github.com/dm-academy/node-svc[node-svc].
...
Navigate to the repo (`cd node-svc`) and check out the 02 branch (matching this lesson)
...
* Go to your Github acccount and create a repository called <my St Thomas ID>-SEIS664. For example, `maty0145-SEIS664`.
...
Create a directory for this section:
...
----
$ git add . -A
$ git commit -m "first section 03 commit" # should be relevant to the changes you made
...
IMPORTANT: *The lab-example repo.* At https://github.com/dm-academy/lab-example you will find a complete set of correctly worked examples, including additional resources and solutions. I encourage you to treat this as a reference, when you are stuck. You of course can simply copy the files and run them, but you may not understand things as well. I recommend building the files as suggested in this tutorial. 

=== Provisioning script

We can automate the process of creating the VM and the firewall rule. In the 01 directory create a script `provision.sh`:
...
In the `01` directory create a bash script `config.sh` to install node, npm, express, and git. Create a script `install.sh` to download the app and initialize node.
...
In the 01 directory create a script `deprovision.sh`.
...
Using your git skills, save and commit the scripts created in this section into your repo.
...
We'll discuss more on that in the next sections.
