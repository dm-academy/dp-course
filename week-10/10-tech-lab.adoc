...
sudo find / -name lynis
   ...
   /usr/share/lynis
   /usr/sbin/lynis
Examine these locations to find the executable lynis*.  The lynis executable is located in /usr/sbin. 
....

Once you know where it got downloaded, run 'audit system' with the --quick option (don't wait for user input).  Lynis will generate hundreds of findings 
that will scroll off your display.  Redirect the lynis output to a file, e.g. lynis_out, so you can view it later.
...
....
sudo find / -name owasp-zap
   /usr/share/doc/owasp-zap
   /usr/share/owasp-zap
   /usr/share/owasp-zap/debian/owasp-zap
   /usr/share/owasp-zap/debian/owasp-zap/usr/share/owasp-zap
   /usr/bin/owasp-zap
Examine these locations to find the executable zap.sh*.  The zap.sh executable is located in /usr/share/owasp-zap.

Examine zap.sh, run it and redirect the output to a file.
sudo /usr/share/owasp-zap/zap.sh -cmd -quickurl http://localhost:80 > zap_sh_out
....
